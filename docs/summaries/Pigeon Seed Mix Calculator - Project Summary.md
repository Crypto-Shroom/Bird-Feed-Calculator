...
```
python-package/
└── pigeon_mix_calculator/
    ├── README.md                    (Technical overview)
    ├── expanded_ingredients.py      (Extended ingredient & herb data)
    ├── pigeon_mix_calculator.py     (Main CLI/programmatic entry point)
    ├── pigeon_mix_calculator_v2.py  (Enhanced version with herb guidance)
    ├── test_calculator.py           (Regression scenarios)
...
Detailed quick-start and user guides now live under `docs/python-cli/`, and research citations are centralized in `docs/research/`.

## Usage Examples

### Interactive Mode
```bash
cd python-package/pigeon_mix_calculator
