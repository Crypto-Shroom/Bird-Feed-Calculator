...
**Location**: `Home.tsx` lines 238-283
...
**Location**: `Home.tsx` NutritionCard component
...
**Status**: IMPLEMENTED
- Profile exists in birds.ts for pigeons
...
**Location**: `birds.ts` - BIRD_PROFILES.pigeon.pet
...
**Location**: `Home.tsx` lines 203-218
...
**Location**: 
- Inventory display: `Home.tsx` lines 304-306
- Mix table: `Home.tsx` lines 430-445 (preparation notes below table)
...
**Location**: `Home.tsx` - Detailed Analysis tab (last section)
...
**Location**: 
- Safety module: `safety.ts` - isToxicRaw() function
- Inventory display: `Home.tsx` lines 310-316 (red alert box)
- Data: `data.ts` - ingredient definitions with toxic flags
...
**Location**: `calculator-multi-bird.ts` lines 70-80
...
**Location**: `Home.tsx` lines 135-154
**Colors**: 
- Pigeon: bg-blue-500
- Parrot: bg-green-500
...
**Location**: `birds.ts` - BIRD_PROFILES object
...
**Location**: 
- Filtering: `Home.tsx` lines 102-115 (sortedIngredients, incompatibleMatches)
- Dropdown sections: `Home.tsx` lines 262-282
- Inventory warnings: `Home.tsx` lines 310-312
...
**Location**: `bird-safety.ts` - BIRD_TOXIC_FOODS object
...
**Location**: `data.ts` - HERB_RECOMMENDATIONS object
...
**Location**: `calculator-multi-bird.ts` - checkMissingCategories()
...
**Location**: `/home/ubuntu/RESEARCH_REFERENCES.md`
**Key Sources**:
...
**Location**: `data.ts` - INGREDIENTS object
**Ingredients Include**:
...
- `data.ts`: Ingredient database, profiles (pigeon-only), herbs
- `birds.ts`: Bird types, bird-specific profiles, bird info (colors, names)
- `bird-safety.ts`: Bird-specific toxicity data, ingredient compatibility
- `safety.ts`: Toxic legume warnings, preparation instructions
- `calculator-multi-bird.ts`: Multi-bird calculator with bird-specific logic

**UI Layer** (`client/src/pages/`):
- `Home.tsx`: Main calculator interface with bird selector, all features
