...
function HomeContent() {
  ...
  const currentProfile = birdProfiles[situation] || Object.values(birdProfiles)[0];
  ...
  useEffect(() => {
    const calculator = new MultibirMixCalculator(inventory, situation, selectedBird);
  ...
  const sortedIngredients = useMemo(() => {
    ...
    const filtered = all.filter(ing => isIngredientCompatible(ing, selectedBird));
    ...
    <div className="min-h-screen bg-background font-sans">
      ...
      <main className="container py-12 -mt-20 relative z-30">
        ...
        <div className="mb-8 pb-6 border-b border-border/50">
          <h3 className="text-sm font-medium text-muted-foreground mb-3">Select Bird Type</h3>
                  ...
                  <SelectContent>
                    ...
                    <ScrollArea className="h-[300px]">
                      {sortedIngredients.length > 0 && (
                        <>
                          <div className="px-2 py-1.5 text-xs font-semibold text-muted-foreground">Compatible</div>
                      ...
                      {incompatibleMatches.length > 0 && (
                        <>
                          <div className="px-2 py-1.5 text-xs font-semibold text-muted-foreground border-t mt-2 pt-2">Not Recommended</div>
                          {incompatibleMatches.map(ing => (
                            <SelectItem key={ing} value={ing} disabled={!!inventory[ing]} className="opacity-50">
                ...
                <ScrollArea className="h-full px-6 pb-6">
                  <div className="space-y-4 pt-2">
                    {Object.entries(inventory).map(([name, amount]) => {
                      ...
                      const isIncompatible = !isIngredientCompatible(name, selectedBird);
                      const hasWarning = toxicInfo || birdToxic || isIncompatible;
                      return (
                      <div key={name} className={cn("flex items-center gap-3 group p-3 rounded-lg border", hasWarning ? "bg-red-50 border-red-300" : "border-transparent")}>
                        <div className="flex-1">
                          <div className="flex justify-between mb-1">
                            <span className="font-medium capitalize text-sm">
                              ...
                              {birdToxic && (
                                <span className="text-xs text-red-600 font-semibold ml-2">WARNING TOXIC</span>
                              )}
                              {isIncompatible && !birdToxic && (
                                <span className="text-xs text-orange-600 font-semibold ml-2">Not recommended</span>
                              )}
                            </span>
                          ...
                          {(toxicInfo || birdToxic || isIncompatible) && (
                            <div className="mb-2 p-2 bg-red-100 border border-red-300 rounded text-xs text-red-900 font-semibold">
                              {birdToxic && `WARNING: ${birdToxic.toxin}. ${birdToxic.message}`}
                              {toxicInfo && !birdToxic && `WARNING: ${toxicInfo.toxin}. ${toxicInfo.message}`}
                              {isIncompatible && !birdToxic && !toxicInfo && `Not recommended for ${birdInfo.name}`}
                            </div>
                          )}
