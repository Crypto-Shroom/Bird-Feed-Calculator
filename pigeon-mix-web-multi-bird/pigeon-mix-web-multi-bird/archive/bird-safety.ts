...
export const BIRD_TOXIC_FOODS: Record<BirdType, Record<string, ToxicFood>> = {
  pigeon: {
    ...
    bean: {
      name: "Bean (generic)",
      toxin: "Hemagglutinin (Lectin)",
      message: "Check if the type of bean contains hemagglutinin. Only safe if cooked.",
      severity: "warning"
    }
  },
  parrot: {
    ...
    salt: {
      name: "Salt",
      toxin: "Sodium",
      message: "Excessive salt can cause kidney damage and electrolyte imbalance.",
      severity: "warning"
    },
    onion: {
      name: "Onion",
  ...
  african_grey: {
    ...
    chocolate: {
      name: "Chocolate",
      toxin: "Theobromine",
      message: "Toxic to African Greys. Can cause tremors, seizures, and death.",
      severity: "critical"
    },
    salt: {
      name: "Salt",
...
export const INGREDIENT_COMPATIBILITY: Record<string, IngredientCompatibility> = {
  wheat: {
    compatible_with: ["pigeon", "parrot", "african_grey", "budgie", "canary", "chicken"],
    incompatible_with: [],
  ...
  kidney_beans: {
    compatible_with: [],
    incompatible_with: ["pigeon", "parrot", "african_grey", "budgie", "canary", "chicken"],
...
export function isIngredientCompatible(ingredient: string, bird: BirdType): boolean {
...
export function getIncompatibilityReason(ingredient: string, bird: BirdType): string | null {
  const compat = INGREDIENT_COMPATIBILITY[ingredient];
...
export function getToxicFoodForBird(ingredient: string, bird: BirdType): ToxicFood | null {
