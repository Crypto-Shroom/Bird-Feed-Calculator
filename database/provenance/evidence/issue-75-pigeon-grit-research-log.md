...
| D | Reconcile current repository implementation and historical provenance before any active change. | Canonical source ledger, care claims, active `BIRD_CARE`, and Home rendering. | Completed. The VCA passage addresses mechanical-grit controversy and excess; it does not assess individual voluntary-use selection. Per owner direction, the existing voluntary-use guidance remains unchanged. |
...
| --- | --- | --- |
| The active runtime provides broad voluntary-use grit guidance, while VCA discusses a narrower mechanical-grit controversy and cautions against excess. | `v3-webapp/client/src/lib/birds.ts`; `vca-pigeon-dove-feeding` | This is a scope difference, not a basis to change the owner-directed voluntary-use guidance. The VCA source does not assess whether pigeons self-regulate voluntary intake; no runtime wording changed in this provenance-only branch. |
