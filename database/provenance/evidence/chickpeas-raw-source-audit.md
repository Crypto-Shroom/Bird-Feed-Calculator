...
| --- | --- | --- | --- |
| `vca-pigeon-dove-feeding` | VCA, *Feeding Pigeons and Doves* | The source permits small quantities of wholesome human food within a balanced pigeon/dove diet, but does not name chickpeas or raw pulses. | Pigeon-specific diet context only. It is insufficient for an outcome and leaves the pigeon record `unresolved` after the documented replacement search. |
| `happy-cockatoo-parrot-sprouts-2012` | Michelle “Elle” Magnon, *Soaks and Sprouts: A Nutritional Powerhouse for Parrots* | The parrot-focused source names garbanzo beans/chickpeas and states that legumes are not safe raw; it requires sprouting to visible root tails. Its author describes more than 20 years of parrot care and newsletter contributions, but it is non-clinical. | Direct group-specific basis for the parrot `requires_preparation` boundary only; it does not establish a cooking method, portion, formula inclusion, runtime approval, or another species outcome. |
...
The final record retains **one `unresolved` outcome**: pigeon. The replacement search included pigeon-specialist feeding guidance, established pigeon rescue and pet-pigeon domains, veterinary-domain searches, regional chickpea terminology, and academic terminology. It did not identify a sourceable direct pigeon chickpea/raw-pulse statement that met the project's species-specific standard; inaccessible social posts and unverified snippets were excluded. Parrot, African Grey, budgie, and canary are `requires_preparation`, each with a documented direct or species/group-specific boundary. Chicken remains `limited` to the cited controlled broiler-ration study. No cross-species suitability, runtime behavior, formula, inventory instruction, preparation method, or visitor-visible copy has been introduced.
