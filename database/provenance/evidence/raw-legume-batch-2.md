...
Every food/form received a first species-specific search for every supported bird. Where that first pass did not establish a direct outcome, a second targeted search using additional terminology and source types was recorded in [`issue-145-research-log.md`](./issue-145-research-log.md). A subsequent compound-level pigeon applicability review is recorded in [`issue-145-pigeon-compound-research.md`](./issue-145-pigeon-compound-research.md). It found no exact pigeon raw-form exposure outcome; after reviewing that gap and the documented compound concerns, the owner explicitly adopted a precautionary `avoid` policy for the pigeon outcome of all five exact raw dried forms. The ledger retains this as owner policy—not direct toxicity evidence. Results are incorporated in `food-reviews.json` and linked from the protected raw-legume coverage ledger.

| Food/form | Pigeon | Parrot | African Grey | Budgie | Canary | Chicken |
| --- | --- | --- | --- | --- | --- | --- |
| Raw dried lima beans | `avoid`* | `requires_preparation` | `requires_preparation` | `requires_preparation` | `requires_preparation` | `requires_preparation` |
| Raw dried fava/broad beans | `avoid`* | `requires_preparation` | `requires_preparation` | `requires_preparation` | `requires_preparation` | `limited` |
| Raw dried black beans | `avoid`* | `requires_preparation` | `requires_preparation` | `requires_preparation` | `requires_preparation` | `unresolved` |
| Raw dried pinto beans | `avoid`* | `requires_preparation` | `requires_preparation` | `requires_preparation` | `requires_preparation` | `requires_preparation` |
| Raw dried navy beans | `avoid`* | `requires_preparation` | `requires_preparation` | `requires_preparation` | `requires_preparation` | `requires_preparation` |

## Evidence assessment

For **pigeons**, neither the direct VCA pigeon/dove guidance nor Palomacy’s pigeon-feeding guidance identifies any of these exact raw dried bean forms. A renewed source audit did locate direct primary crop-content evidence that free-ranging feral pigeons consumed **broad beans (*Vicia faba*)** in a field-foraging context.[6] However, that study does not identify bean maturity or processing, so it cannot establish the distinct **raw dried** fava/broad-bean form. The direct-evidence limitation remains: renewed searches found no accessible exact pigeon/form evidence for lima, fava, black, pinto, or navy beans, and no parrot or chicken result is inferred. Separately, the owner has adopted `avoid` for the pigeon outcome of all five exact raw dried forms as a precautionary policy because compound concerns are documented and safe pigeon use is not clearly established. This is not presented as direct pigeon toxicity evidence.
...
| `sources.json` | Added the direct chicken study records, companion-bird black-bean preparation source, direct feral-pigeon broad-bean crop-content study, five compound/pigeon-context sources, and the owner-approved pigeon precautionary-policy source; clarified the existing For the Birds source’s named-bean scope. |
