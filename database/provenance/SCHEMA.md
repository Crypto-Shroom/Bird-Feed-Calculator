...
| Explicitly justified cross-species inference | A source written for one bird cannot **silently** permit a food for another. However, where direct bird/form evidence is absent, a row may use `related_species` only when its rationale explicitly identifies the shared ingredient identity and relevant constituent or preparation boundary, explains why the exposure/form comparison is materially applicable, cites the supporting source and locator, names the target bird, and states what remains unproven. The inference must remain bounded to the stated outcome; it cannot establish a dose, formula, complete ration, therapeutic effect, runtime approval, or a different preparation. If those conditions are not met, the row remains `unresolved`. |
