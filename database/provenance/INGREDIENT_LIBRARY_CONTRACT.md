...
The first evidence batch now records four additional existing food forms: `wheat` as whole dry grain, `millet` as whole dry seed, `green peas` as fresh lightly cooked peas, and `chickpeas` as plain cooked chickpeas. Each form has six explicit species rows. The outcomes remain deliberately mixed: some species have limited or preparation-required evidence, while other rows remain `unresolved` because the reviewed sources do not establish that exact food/form outcome. The preserved apple record remains the only earlier completed batch. These are research records only; they do not change runtime behavior.
