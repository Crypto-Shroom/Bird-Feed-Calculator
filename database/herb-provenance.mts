/**
 * Canonical herb provenance and automatic-suggestion compatibility registry.
 *
 * This repository-level database file is the human-reviewable source for
 * evidence links and species eligibility. The V3 application imports it through
 * client/src/lib/herb-evidence.ts; do not create a competing UI-side registry.
 */
export type HerbBirdKey = "pigeon" | "parrot" | "african_grey" | "budgie" | "canary" | "chicken";
...
export interface HerbEvidence {
  ...
  compatibleBirds: readonly HerbBirdKey[];
  sourceIds: readonly (keyof typeof HERB_SOURCES)[];
  scope: string;
}

const allSupportedBirds = ["pigeon", "parrot", "african_grey", "budgie", "canary", "chicken"] as const satisfies readonly HerbBirdKey[];
const pigeonsOnly = ["pigeon"] as const satisfies readonly HerbBirdKey[];
...
const pigeonEligible = (sourceIds: HerbEvidence["sourceIds"], scope: string): HerbEvidence => ({
  eligibility: "eligible",
  compatibleBirds: pigeonsOnly,
  sourceIds,
  scope,
});
